...
static int init()
{
    ...
    else
    {
        ...
        //Create window
        gWindow = SDL_CreateWindow( "Richie Rich IUT", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, SCREEN_WIDTH, SCREEN_HEIGHT, SDL_WINDOW_SHOWN );
        if( gWindow == NULL )
        {
    ...
    background = loadTexture( "images/IUT.bmp", gRenderer);
...
static SDL_Texture* loadTexture(char* file, SDL_Renderer *const renderer)
{
...
static void renderText(const char * text, int x, int y, SDL_Color color)
{
    ...
    Message_rect.x = 130;  //controls the rect's x coordinate
...
static int pop_up1()
{
    ...
    const SDL_MessageBoxData messageboxdata = {
        ...
        NULL, /* .window */
        "Welcome", /* .title */
        "Press Yes to get started." ,/* .message */
    ...
    else if(buttonid==0)
    {
        ...
        SDL_Log("selection was %s\n Welcome to the game!!", buttons[buttonid].text);
        const SDL_MessageBoxButtonData buttons3[]={
 ...
 const SDL_MessageBoxColorScheme colorScheme3 = {
        { /* .colors (.r, .g, .b) */
            /* [SDL_MESSAGEBOX_COLOR_BACKGROUND] */
            { 255,   0,   0 },
            /* [SDL_MESSAGEBOX_COLOR_TEXT] */
    ...
    const SDL_MessageBoxData messageboxdata3 = {
        SDL_MESSAGEBOX_INFORMATION, /* .flags */
        NULL, /* .window */
        "Ritchie Rich IUT", /* .title */
        "Do you want to know the instructions?" ,/* .message */
        SDL_arraysize(buttons3), /* .numbuttons */
        buttons3, /* .buttons */
        &colorScheme3 /* .colorScheme */
    };
int buttonid3;
    if (SDL_ShowMessageBox(&messageboxdata3, &buttonid3) < 0) {
        SDL_Log("error displaying message box");
        return 0;
    }
    if (buttonid3 == -1) {
    ...
    const SDL_MessageBoxData messageboxdata4 = {
        ...
        NULL, /* .window */
        "Ritchie Rich IUT", /* .title */
        "1.Passing AB Bank will bring your CGPA back to 4 & add 2000tk in your wallet.\n 2.You will be PENALIZED!!\n a. Breaking rules (getting same number on both dices thrice in a row), landing on properties you shouldn’t visit(Go to VC's office) will land you in the VC's office & result in degradation of your CGPA.\n Good luck on figuring out your way out of VC's office 😉 \n ",/*message*/
...
void renderEverything()
{
    ...
    {
        renderText(text, 130, 440 + 30 * i, color);
        i++;
    }
